...
class SubFormulaShould {
    ...
    void parse_an_empty_element() {
        ...
        assertThat(subformula).isEqualToComparingFieldByFieldRecursively(new SubFormula("(O)", "O", 1));
    }

    @Test
    void parse_a_valued_element() {
        SubFormula subformula = (SubFormula) SubFormula.extract("(O2)3", '(', ')').get();
        assertThat(subformula).isEqualToComparingFieldByFieldRecursively(new SubFormula("(O2)3", "O2", 3));
    }

    @Test
    void parse_an_element_with_parentesis() {
        SubFormula subformula = (SubFormula) SubFormula.extract("(O2(Cl3H2O))3", '(', ')').get();
        assertThat(subformula).isEqualToComparingFieldByFieldRecursively(new SubFormula("(O2(Cl3H2O))3", "O2(Cl3H2O)", 3));
    }
